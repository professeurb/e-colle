...
IP_FILTRE_ADMIN = False # filtrage IP pour l'utilisateur administrateur
...
DB_PASSWORD = 'e-colle' # mot de passe pour se connecter à la base de données
DB_HOST = 'db' # adresse locale de la base de données
DB_PORT = 5432 # port de la BDD, vide par défaut. À renseigner si la BDD se trouve sur un port particulier
IMAGEMAGICK = False # utilisation de ImageMagick pour faire des miniatures de la première page des pdf programmes de colle
ALLOWED_HOSTS = ['localhost'] # liste des noms de domaine autorisés pour accéder à e-colle
